require([
    ...
    arcgisUtils, TimeExtent, MosaicRule, 
    ImageServiceIdentifyTask, ImageServiceIdentifyParameters, 
    Search, esriRequest, webMercatorUtils,
    ...
    function setOperationalLayersVisibility() {
        ...
        var selectedMapLayer = app.webMapItems.operationalLayers.filter(function(d) { return d.title === selectedMapLayerName; })[0];
    ...
    function MainChart(data) {
        ...
        var uniqueTimeValues = data[0].values.map(function(d) { return d.stdTime; });
        var uniqueYearValues = uniqueTimeValues.map(function(d) { return timeFormat(new Date(d)); });
        uniqueYearValues = uniqueYearValues.filter(function(item, pos, self) { return self.indexOf(item) == pos; });
        ...
        var precipData = data.filter(function(d) { return d.key === "Precipitation"; });
        var evapoData = data.filter(function(d) { return d.key === "Evapotranspiration"; });
        var snowpackData = data.filter(function(d) { return d.key === "Snowpack"; });
        var soilMoistureData = data.filter(function(d) { return d.key === "Soil Moisture"; });
        var runoffData = data.filter(function(d) { return d.key === "Runoff"; });
        ...
        var bars = svg.selectAll("bar")
            ...
            .style("opacity", 0.7)
            .attr("x", function(d) { return xScale(d.stdTime) - barWidth / 2; })
            .attr("width", barWidth)
            .attr("y", function(d) { return yScale(d.value); })
            .attr("height", function(d) { return height - margin.top - yScale(d.value); });
        ...
        var haloLines = lineFeatures.append('path')
        ...
        var lines = lineFeatures.append('path')
            ...
            .attr("clip-path", "url(#chartAreaClip)")
            .attr('d', function(d) { return createLine(d.values); })
            .attr('stroke', function(d) {  return getColorByKey(d.key); })
            .attr('stroke-width', 2)
            .attr('fill', 'none');
        ...
        var highlightRefLine = svg.append('line')
            ...
            .style("display", "none")
            .attr("stroke", highlightRefLineColor)
        ...
        var highlightRefLineLabelRect = highlightRefLineLabel.append('rect')
            ...
            .style('opacity', 0.7)
            .style('fill', highlightRefLineColor);
        ...
        function getSummaryDataByTime(time) {
            ...
            var precipDataByTime = summaryDataByTime.filter(function(d) { return d.key === "Precipitation"; });
            var evapoDataByTime = summaryDataByTime.filter(function(d) { return d.key === "Evapotranspiration"; });
            var runoffDataByTime = summaryDataByTime.filter(function(d) { return d.key === "Runoff"; });
            var soildMoistureDataByTime = summaryDataByTime.filter(function(d) { return d.key === "Soil Moisture"; });
            var snowpackDataByTime = summaryDataByTime.filter(function(d) { return d.key === "Snowpack"; });
            var changeInStorageDataByTime = summaryDataByTime.filter(function(d) { return d.key === "ChangeInStorage"; });
        ...
        function setVericalPosition(containerElementClassName, floatingElementClassName, ratio) {
            ...
            var topMarginValue = (ratio >= 0) ?
                (containerElementHeightHalf + (containerElementHeightHalf * ratio)) - floatingElementHeightHalf :
                (containerElementHeightHalf * (1 + ratio)) - floatingElementHeightHalf;
        ...
        function setSummaryDescTextValue(changeInStorageValue, soilmoistureValue, monthName) {
            ...
            var descTextElements1 = [
                ...
                "<span class='soil-moisture-value'>" + soilMoisturePctText,
                "average</span> for",
                monthName + "."
            ];
            ...
            if (legendValue) {
                if (app.isWaterStorageChartVisible) {
                    ...
                    areas = areasG.selectAll(".area-layer")
                        .data(areaChartLayersByLegendValue)
                        .enter().append("path")
                        .attr("class", "area-layer")
                        .attr("d", function(d) {
                            return createArea(d.values);
    ...
    function MonthlyTrendChart(data) {
        ...
        var uniqueYearValues = chartData[0].values[0].values.map(function(d, i) { return d.year; });
        var uniqueMonthValues = chartData[0].values.map(function(d, i) { return d.key; });
        uniqueMonthValues = uniqueMonthValues.filter(function(d) { return d !== "Annual"; });
        ...
        var yScale = d3.scale.linear()
            .range([height - margin.top, 0])
            .domain([0, d3.max(soilMoistureData[0].values.concat(snowpackData[0].values), function(d) { return d.value; })]);
        ...
        var createLineForMonthlyNormals = d3.svg.line()
            .x(function(d) { return xScaleForMonthlyNormals(d.key); })
            .y(function(d) { return yScale(+d.normalizedValue); })
            .interpolate("monotone");

        var precipDataNested = chartData.filter(function(d) { return d.key === "Precipitation"; });
        var evapoDataNested = chartData.filter(function(d) { return d.key === "Evapotranspiration"; });
        var soilMoistureDataNested = chartData.filter(function(d) { return d.key === "Soil Moisture"; });
        var snowpackDataNested = chartData.filter(function(d) { return d.key === "Snowpack"; });
        var runoffDataNested = chartData.filter(function(d) { return d.key === "Runoff"; });
        var changeInStorageDataNested = chartData.filter(function(d) { return d.key === "ChangeInStorage"; });
        ...
        //create features group for each data group
        var featuresForMonthlyNormals = svg.selectAll('features')
            .data(chartData)
            .enter().append('g')
            .attr('class', 'features-for-monthly-normals');

        //append the line graphic
        var linesForMonthlyNormal = featuresForMonthlyNormals.append('path')
            .attr('class', 'monthly-normal-trend-line')
            .attr('d', function(d) { return createLineForMonthlyNormals(getMonthlyNormalsData(d)); })
            .attr('stroke', function(d, i) { return getColorByKey(d.key); })
            .style('opacity', "0.2")
            .attr('stroke-width', 1)
            .attr('fill', 'none');

        function getMonthlyNormalsData(data) {
            return data.values.filter(function(k) { return k.key !== "Annual"; });
        }
        ...
        var tooltipTextForXValue = tooltipGroupForXValue.append("text")
            .attr('class', 'tooltip-text tooltip-text-bold')
        ...
        var overlay = svg.append("rect")
            ...
            .attr("width", width)
            .attr("height", height)
            .on("mouseover", function() { toggleTootip(true); })
            .on("mouseout", function() { toggleTootip(false); })
            .on("mousemove", mousemove);
        ...
        function mousemove() {
            ...
            var chartDataByLayerType = chartData.filter(function(d) { return d.key === dataLayerType; })[0];
        ...
        this.updateChartScale = function() {
            ...
            var getYScaleDomainForSummarizedValues = function() {
                ...
                var chartDataByLayerType = chartData.filter(function(d) { return d.key === dataLayerType; })[0];
                var annualTotalData = chartDataByLayerType.values.filter(function(d) { return d.key === "Annual"; })[0];
                var monthlyData = chartDataByLayerType.values.filter(function(d) {return d.key !== "Annual";});
            ...
            var getYScaleDomainForMonthlyValues = function() {
                var chartDataByLayerType = data.filter(function(d) { return d.key === dataLayerType; })[0];
            ...
            yAxisG.transition().duration(1000).ease("sin-in-out").call(yAxis);
            lines.transition().duration(1000).attr('d', function(d) { return createLine(d.values); });
            linesForMonthlyNormal.transition().duration(1000).attr('d', function(d) { return createLineForMonthlyNormals(getMonthlyNormalsData(d)); });
        ...
        this.resize = function() {
            ...
            lines.attr('d', function(d) { return createLine(d.values); });
            linesForMonthlyNormal.attr('d', function(d) { return createLineForMonthlyNormals(getMonthlyNormalsData(d)); })
        ...
        this.getChangeInStorageDataByMonth = function(fullMonthName) {
            var changeInStorageDataByMonth = changeInStorageDataNested[0].values.filter(function(d) { return d.key === fullMonthName; });
            return changeInStorageDataByMonth[0];
        }

        this.getSoilMoistureDataByMonth = function(fullMonthName) {
            var soilMoistureDataByMonth = soilMoistureDataNested[0].values.filter(function(d) { return d.key === fullMonthName; });
    ...
    function convertChartDataToCSV(objArray){
        ...
        // get column headers
        app.mainChart.rawData.forEach((item, idx)=>{
        ...
        app.mainChart.rawData[0].values.forEach((item, idx)=>{
            var rowData = [
                timeFormatFullMonthName(new Date(item.stdTime)),
                item.value,
                app.mainChart.rawData[1].values[idx].value,
                app.mainChart.rawData[2].values[idx].value,
                app.mainChart.rawData[3].values[idx].value,
                app.mainChart.rawData[4].values[idx].value,
                app.mainChart.rawData[5].values[idx].value
            ];
